...
from transformers import Dinov2Model, DPTModel, AutoModel, Dinov2WithRegistersModel
...
#         # Offset prediction: 2 offsets (x,y) for each position in the kernel
#         self.offset_conv = nn.Conv2d(in_channels, 2*kernel_size*kernel_size, 
#                                     kernel_size=3, padding=1, bias=bias)
        
#         # Initialize the offset weights to zero so we start with regular convolution
#         nn.init.constant_(self.offset_conv.weight, 0.)
#         if bias:
#             nn.init.constant_(self.offset_conv.bias, 0.)
            
#         # Modulation mask (optional, used in DCNv2)
...
#     def _get_p(self, h, w, device):
#         """Generate regular grid sampling locations"""
#         y, x = torch.meshgrid(
#             torch.arange(0, h, device=device),  # [h]
#             torch.arange(0, w, device=device)   # [w]
#         )  # y: [h, w], x: [h, w]
        
#         # Flatten and repeat for each position in the kernel
#         y = y.flatten().view(1, 1, h, w).repeat(1, self.kernel_size*self.kernel_size, 1, 1)  # [1, k*k, h, w]
#         x = x.flatten().view(1, 1, h, w).repeat(1, self.kernel_size*self.kernel_size, 1, 1)  # [1, k*k, h, w]
        
#         # Stack x and y coordinates
#         p = torch.cat([x, y], 1)  # [1, 2*k*k, h, w]
#         return p
...
#         # Regular grid points
#         p = self._get_p(H, W, device)  # [1, 2*k*k, H, W]
        
#         # Predict offsets
#         offsets = self.offset_conv(x)  # [B, 2*k*k, H, W]
        
#         # Apply offsets to regular grid
#         p = p.expand(B, -1, -1, -1)  # [B, 2*k*k, H, W]
#         p = p + offsets  # [B, 2*k*k, H, W]
        
#         # Predict modulation masks (for DCNv2)
#         modulation = torch.sigmoid(self.modulation_conv(x))  # [B, k*k, H, W]
        
#         # For a full implementation, we would now use the grid p to sample from x
#         # This would require a custom CUDA operation for efficiency
#         # For this example, we'll approximate by using the standard convolution
#         # and applying the modulation weights
        
#         # Standard convolution
#         out = self.conv(x)  # [B, out_channels, H', W'] where H',W' may be affected by stride
        
#         # Apply modulation factor
#         modulation = modulation.view(B, 1, self.kernel_size*self.kernel_size, H, W)  # [B, 1, k*k, H, W]
#         modulation = modulation.mean(dim=2, keepdim=False)  # [B, 1, H, W] - Approximate effect
#         out = out * modulation  # [B, out_channels, H', W']
...
class DirectionAwareDecoder(nn.Module):
    def __init__(self, in_channels, angle_num=4):
        ...
        # self.deform_conv = DeformConv2d(conv_channels, conv_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        # apply modulation
        B, C, H, W = x.shape
        x_modulated = x * modulation.view(B, 1, 9, H, W).mean(dim=2)  # 近似调制
        
        # apply deformable convolution
        x_deformed = self.deform_conv(
            x_modulated, 
            offsets  # 必须传入offset参数
        )  # [B, 256, H, W]

        # Deformable alignment
        # x = self.deform_conv(x)  # [B, 256, H, W]  
        # do not residual connection here (+ x)
        ...
        pooled = torch.cat(angle_features, dim=3)  # [B, 256, 1, 4] - 因为有4个角度(0,45,90,135)
...
def generate_heatmap(positions, img_size=(224, 224), sigma=4.0):
    ...
    # Normalize heatmaps (optional)
    # heatmaps = heatmaps / torch.sum(heatmaps.view(B, num_joints, -1), dim=-1, keepdim=True).view(B, num_joints, 1, 1)
...
class KinematicsLayer(nn.Module):
    ...
    def forward(self, angles):
        ...
        for i in range(angles.size(1)):  # num_joints
            current_angle = current_angle + angles[:,i]  # 修改为创建新张量
            delta_x = self.links[i] * torch.cos(current_angle)  # [B]
            delta_y = self.links[i] * torch.sin(current_angle)  # [B]
            current_pos = current_pos + torch.stack([delta_x, delta_y], -1)  # 修改为创建新张量
            positions.append(current_pos.clone())  # 确保每次都是新张量
            
        positions = torch.stack(positions, 1)  # [B, num_joints, 2]
        return positions.view(B, -1)  # 展平为[B, num_joints*2]
